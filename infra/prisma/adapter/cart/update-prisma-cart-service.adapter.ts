import { CartEntity } from "@/domain/entities/cart/cart.entity";
import { updatePrismaCartService } from "@/infra/prisma/services/cart/update-prisma-cart.service";
import { cartEntityToCartDatabaseMapper } from "@/infra/prisma/mappers/cart/cart-entity-to-cart-database.mapper";
import { cartDatabaseToCartEntity } from "@/infra/prisma/mappers/cart/cart-database-to-cart-entity";

export async function updatePrismaCartServiceAdapter(
  cart: CartEntity
): Promise<CartEntity> {
  const cartUpdated = await updatePrismaCartService(
    cartEntityToCartDatabaseMapper(cart)
  );

  return cartDatabaseToCartEntity(cartUpdated);
}
